<template>
  <div class="search" v-if="isSearch">
    <van-search
      shape="round"
      placeholder="搜索医生、科室、医院"
      :show-action="true"
      v-model="searchValue"
      @cancel="onCancel"
      @search="onSearch"
      @clear="onClear"
    />
    <div class="search-history" v-show="hasValue">
      <div>搜索记录</div>
      <ul class="histor-list">
        <li>王玲</li>
        <li>衡水</li>
      </ul>
    </div>
    <div class="hospital" v-if="hospitalData.length > 0">
      <Hospital :hospitals="hospitalData" />
    </div>
    <div class="doctors" v-if="doctorList.length > 0">
      <DoctorList :doctorList="doctorList" />
    </div>
    <div class="empty">
      <button class="empty-btn">清空搜索记录</button>
    </div>
  </div>
</template>

<script lang="ts">
import { defineComponent, reactive, toRefs } from 'vue'
import { finSearchContent } from '../../common/api'
import Hospital from '../Hospital/Index.vue'
import DoctorList from '../DoctorList/Index.vue'
import { Toast } from 'vant'
export default defineComponent({
  components: {
    Hospital,
    DoctorList,
  },
  props: {
    isSearch: {
      type: Boolean,
      default: false,
    },
  },
  setup(props, ctx) {
    const state = reactive({
      searchValue: '',
      hospitalData: [],
      doctorList: [],
      depts: [],
      hasValue: true,
    })
    const onCancel = () => {
      ctx.emit('cancel')
    }
    const onClear = () => {
      state.hospitalData = []
      state.doctorList = []
      state.depts = []
      state.hasValue = true
    }
    const onSearch = () => {
      getSearchContent()
    }
    const getSearchContent = async () => {
      const params = {
        param: state.searchValue,
      }
      const { data, success } = await finSearchContent(params)
      if (success) {
        const { depts, doctors, hospitals } = data
        state.hospitalData = hospitals
        state.doctorList = doctors
        state.hasValue = false
        if (!depts.length && !doctors.length && !hospitals.length) {
          Toast('未查询到内容！')
        }
      }
    }
    return {
      ...toRefs(state),
      onCancel,
      onSearch,
      onClear,
    }
  },
})
</script>

<style lang="scss" scoped>
.doctor-component .doctor-list {
  padding: 0 !important;
}
.search {
  padding: 20px;
}
.search-history {
  margin-top: 20px;
  color: #aaa;
}
.histor-list {
  display: flex;
  align-items: center;
  flex-wrap: wrap;
  margin-left: -20px;
  margin-top: 10px;
  li {
    color: #666;
    background-color: #ededed;
    border-radius: 8px;
    font-size: 12px;
    padding: 2px 6px;
    margin-left: 20px;
  }
}
.empty {
  display: flex;
  justify-content: center;
  margin-top: 10px;
  .empty-btn {
    border-radius: 50px;
    border: 1px solid #ccc;
    font-size: 14px;
    color: #999;
    padding: 4px 18px;
  }
}
</style>